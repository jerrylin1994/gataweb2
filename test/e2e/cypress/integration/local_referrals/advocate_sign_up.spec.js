  ...
  Cypress.testFilter( [ "@smoke" ], () => {
    context( "Advocate sign up via email, advocate table content, and activity log test cases", () => {
      it( "Part 1 - Should be able to send an email advocate invite", function() {
        ...
        cy.contains( `Sent invite to ${ advocate_name } at ${ advocate_email }` )
          .should( "be.visible" )
        ...
        cy.wrap( null )
          .then( () => {
            ...
            assert.equal( tableRowText.sent_to, advocate_email )
        ...
        cy.task( "checkEmail", { query: `${ advocate_name }, Earn Rewards By Referring Your Friends Invite them to book with us. When they do, you’ll both earn a ${ friend_reward_name } after their first completed appointment/project/purchase/service! from: noreply@my-referral.co`, email_account: "email1" } )
          .then( ( email ) => {
      ...
      it( "Part 2 - Should be able to sign up as an advocate via invite email", () => {
        cy.readFile( "cypress/helpers/local_referrals/advocate.json" )
          .then( ( data ) => {
            ...
            cy.task( "getEmailElementAttribute", { email_id: data.email_id, email_account: "email1", element_text: "Invite Friends", element_attribute_name: "href", element_tag: "a" } )
      ...
      it( "Part 3 - Should see correct activity log for invited and registered advocate", () => {
        ...
        cy.contains( `Sent advocate invite to ${ advocate_name } at ${ advocate_email }` )
          .should( "be.visible" )
        cy.contains( activity_date )
          .should( "be.visible" )
      ...
      it( "Part 4 - Should see signed up advocate in registered table", () => {
        ...
        cy.wrap( null )
          .then( () => {
            assert.equal( tableRowText.advocate, `${ advocate_name }${ advocate_email }` )
