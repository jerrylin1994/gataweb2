  ...
  context( "Send new message test cases", () => {
    before( () => {
      ...
      cy.task( "getNodeIndex" )
        .then( ( index ) => {
          cy.log( base.getTwilioNumber( index ) )
          const merchant_name = `Test Automation Machine ${ index } Twilio`
          base.removeTwilioNumber2( "Test Automation Manual Booking" )
          // base.deleteMerchants(`Test Automation Send Message`)
          // base.deleteMerchants(`Test Automation blah ${index}`)
    ...
    Cypress.testFilter( [], () => {
      it.skip( "Should see correct activity log for sending message to new contact", function() {
